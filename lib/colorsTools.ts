...
export const factorColor = (color: string, factor?: number, focus?: number) => {
  factor ??= 0.5;
  focus ??= 0x808080;
  const focus_r = (focus >> 16) & 0xff;
  const focus_g = (focus >> 8) & 0xff;
  const focus_b = focus & 0xff;

  let [r, g, b] = colorToRGB(color);
  r = focus_r + (r - focus_r) * factor;
  g = focus_g + (g - focus_g) * factor;
  b = focus_b + (b - focus_b) * factor;
...
export const dullColor = (color: string, factor?: number) => {
  ...
  return factorColor(color, factor, 0x808080);
};
